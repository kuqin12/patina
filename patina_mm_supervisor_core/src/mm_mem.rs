...
// Re-export shared types from the common allocator crate.
pub use patina_internal_mm_alloc::{PageAllocError, PageAllocatorBackend, PoolAllocator};

// ============================================================================
// Constants
// ============================================================================

/// Standard UEFI page size (4 KB). Re-exported from shared crate.
pub use patina_internal_mm_alloc::PAGE_SIZE;
...
// PageAllocError is re-exported from patina_internal_mm_alloc above.
    ...
    fn apply_data_page_attributes(&self, addr: u64, num_pages: usize, _alloc_type: AllocationType) {
        ...
        if let Some(ref mut pt) = *pt_guard {
            ...
            if _alloc_type == AllocationType::Supervisor {
                ...
                attributes = attributes | MemoryAttributes::Supervisor; // Ensure not writable by user code
            }
...
// ============================================================================
// PageAllocatorBackend implementation for PageAllocator
// ============================================================================

impl PageAllocatorBackend for PageAllocator {
    ...
    fn free_pages(&self, addr: u64, num_pages: usize) -> Result<(), PageAllocError> {
        PageAllocator::free_pages(self, addr, num_pages)
    }

    fn is_initialized(&self) -> bool {
        PageAllocator::is_initialized(self)
    }
}
...
///
/// This uses the shared [`PoolAllocator`] from `patina_internal_mm_alloc`,
/// parameterized on the supervisor's [`PageAllocator`].
#[global_allocator]
static GLOBAL_ALLOCATOR: PoolAllocator<PageAllocator> = PoolAllocator::new(&PAGE_ALLOCATOR);
