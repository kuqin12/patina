...
/// through the `unsafe extern "efiapi"` calling convention.
///
/// ## Parameters
///
/// - `handler_type` — The GUID that triggered this handler (same as the registration GUID).
/// - `comm_buffer` — Pointer to the communication data (may be null for async MMIs).
/// - `comm_buffer_size` — Mutable pointer to the communication buffer size.
///
/// ## Returns
///
/// An [`efi::Status`] following the standard `MmiManage` return protocol.
pub type InternalMmiHandler = fn(
...
struct MmiHandler {
    ...
    kind: HandlerKind,
    /// Monotonic ID used as the dispatch handle for unregistration.
    id: usize,
...
impl MmiDatabase {
    ...
    // =========================================================================
    // Registration — external (driver) handlers
    // =========================================================================
    ...
    pub fn mmi_handler_register(
        ...
        let mmi_handler = MmiHandler {
            kind: HandlerKind::External(handler),
            id,
            to_remove: false,
        };
        ...
        log::debug!(
            "Registered external MMI handler id={} for {:?}",
            id,
            handler_type,
        );
        Ok(handle)
    ...
    // =========================================================================
    // Registration — internal (core) handlers
    // =========================================================================
    ...
    pub fn register_internal_handler(
        ...
        let mmi_handler = MmiHandler {
            kind: HandlerKind::Internal(handler),
            id,
            to_remove: false,
        };
        ...
        let handle = id as efi::Handle;
        log::debug!(
            "Registered internal MMI handler id={} for {:?}",
            id,
            handler_type,
        );
        Ok(handle)
    ...
    fn dispatch_handler_snapshot(
        ...
        for handler in handlers {
            let status = match handler.kind {
                ...
                HandlerKind::Internal(fn_ptr) => {
                    fn_ptr(guid_ref, comm_buffer, comm_buffer_size)
                }
            };
