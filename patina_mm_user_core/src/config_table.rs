...
use crate::mm_services::get_mm_system_table;

// =============================================================================
// Global instance
// =============================================================================

/// Global configuration table database used by the MM System Table.
pub static GLOBAL_CONFIG_TABLE_DB: MmConfigurationTableDb = MmConfigurationTableDb::new();
...
impl MmConfigurationTableDb {
    ...
    pub fn install_configuration_table(
        ...
        match (existing_idx, table.is_null()) {
            ...
            (Some(idx), false) => {
                ...
                log::debug!(
                    "MmInstallConfigurationTable: modified {:?}",
                    guid
                );
            }
            ...
            (Some(idx), true) => {
                ...
                log::debug!(
                    "MmInstallConfigurationTable: deleted {:?}",
                    guid
                );
            }
            // No match, table non-null → add
            (None, false) => {
                inner.entries.push(efi::ConfigurationTable {
                    vendor_guid: *guid,
                    vendor_table: table,
                });
                log::debug!(
                    "MmInstallConfigurationTable: added {:?}",
                    guid
                );
            }
        ...
        Self::publish_to_system_table(&mut inner);
    ...
    fn publish_to_system_table(inner: &mut ConfigTableInner) {
        let mmst = get_mm_system_table();
        if mmst.is_null() {
            return;
        }
        ...
        if inner.entries.is_empty() {
            // SAFETY: We hold the Mutex, and `mmst` was initialised by
            ...
            let boxed: Box<[efi::ConfigurationTable]> =
                inner.entries.clone().into_boxed_slice();
            let len = boxed.len();
