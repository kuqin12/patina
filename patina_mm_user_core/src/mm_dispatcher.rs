...
use patina::{boot_services::c_ptr::CPtr, pi::hob::Hob};
use patina_internal_depex::{AssociatedDependency, Depex};
use r_efi::efi;
use spin::Mutex;

use crate::{
    MM_SUPERVISOR_CORE_GUID, MM_SUPERVISOR_DEPEX_HOB_GUID,
    MM_SUPERVISOR_HOB_MEMORY_ALLOC_MODULE_GUID, MM_SUPERVISOR_USER_GUID,
    mmi::MmiDatabase,
...
pub struct MmDispatcher {
    /// Tracks whether the dispatcher is currently executing (prevents re-entrancy).
...
impl MmDispatcher {
                    ...
                    if let Hob::GuidHob(guid_hob, data) = next_hob {
                        if guid_hob.name == MM_SUPERVISOR_DEPEX_HOB_GUID {
                            ...
                            if data.is_empty() {
                                ...
                                // Check to make sure the name matches the expected depex HOB GUID before parsing
                                let depex_hob_data = data.as_ref() as *const [u8] as *const DepexHobData;
                                ...
                                log::info!("  Parsed depex HOB {:#x?} for driver {:?} at {:#x?}: expression length = {}",
    ...
    fn dispatch_drivers(
        ...
        loop {
            // Merge protocols from both the original protocol_db (for depex evaluation)
            // and the global protocol DB (populated by drivers via the MM System Table).
            let mut registered_protocols = protocol_db.registered_protocols();
            registered_protocols.extend(crate::mm_services::GLOBAL_PROTOCOL_DB.registered_protocols());
            ...
            for driver in ordered {
                ...
                type MmDriverEntryPoint =
                    unsafe extern "efiapi" fn(efi::Handle, *const c_void) -> efi::Status;
                let entry_fn: MmDriverEntryPoint =
                    unsafe { core::mem::transmute(driver.entry_point) };

                let status = unsafe {
                    entry_fn(core::ptr::null_mut(), mm_system_table)
                };

                if status == efi::Status::SUCCESS {
                    log::info!("  Driver {:?} returned SUCCESS.", driver.file_name);
                    total_dispatched += 1;
                } else {
                    log::warn!(
